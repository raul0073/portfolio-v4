				...
				<ul className="grid grid-cols-3 md:grid-cols-7 gap-4 md:gap-8 py-8">
					{programingIcons.map((icon, i) => {
						return (
							<li
								key={`${icon.name}-${i}`}
								className="dev-icon group relative rounded-md  flex justify-center  dark:hover:bg-appYellow hover:bg-appBlue  border-bg-light/10 p-2 transition-transform duration-300 ease-in-out ">
								<i
								...
								<motion.span
									...
									className="absolute text-xs uppercase text-nowrap -bottom-4 text-zinc-900 dark:text-bg-light  hidden group-hover:flex">
									{icon.name}
								</motion.span>
							</li>
						);
